...
public interface CourseTeacherRepository extends JpaRepository<CourseTeacher, Long> {
    ...
    @Query("select ct from CourseTeacher ct where ct.course.id=?1 and ct.standard.id=?2 and ct.active=true order by ct.standard.id ")
    List<CourseTeacher> findByCourseAndStandardId(Long courseId,Long standardId);
