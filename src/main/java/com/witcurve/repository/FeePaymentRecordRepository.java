...
public interface FeePaymentRecordRepository extends JpaRepository<FeePaymentRecord, Long> {
    ...
    @Query("select fpr from FeePaymentRecord fpr join StudentStandard ss on ss.student.id = fpr.studentFeeStructure.student.id where ss.standard.id = ?1 and ss.active = true and fpr.studentFeeStructure.session.id = ?2 order by ss.rollNo, fpr.createdDate")
