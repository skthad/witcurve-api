...
public interface ExamCourseDetailsRepository extends JpaRepository<ExamCourseDetails, Long> {
    ...
    @Query("Select distinct(ecd.course) from ExamCourseDetails ecd where ecd.gsd.grade = ?1 and ecd.course.schoolInfo.id = ?2 and ecd.course.courseType ='SCHOLASTIC' and ecd.date between ?3 and ?4 order by ecd.course.elective asc, ecd.course.displayName asc")
    List<Course> findScholasticCourse(Grade grade, Long schoolInfoId, LocalDate startDate, LocalDate endDate);
