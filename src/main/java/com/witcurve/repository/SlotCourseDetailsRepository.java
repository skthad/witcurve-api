...
public interface SlotCourseDetailsRepository extends JpaRepository<SlotCourseDetails, Long> {
    ...
    @Query("Select scd from SlotCourseDetails scd where scd.courseTeacher.teacher.id=?1 and scd.courseTeacher.standard.id=?1 and scd.gsd.exam is null and scd.deleted=false and scd.gsd.status = 'ACTIVE' order by scd.dayOfWeek asc, scd.gsd.start asc")
    ...
    @Query("select distinct scd.courseTeacher.teacher.id from SlotCourseDetails scd where " +
        "scd.courseTeacher.active = true and scd.deleted=false and scd.courseTeacher.course.omitSlotConflict=false and " +
