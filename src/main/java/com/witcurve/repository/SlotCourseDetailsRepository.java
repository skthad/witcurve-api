...
public interface SlotCourseDetailsRepository extends JpaRepository<SlotCourseDetails, Long> {
    ...
    @Query("Select scd from SlotCourseDetails scd where scd.gsd.standard.id=?1 and scd.gsd.exam is null order by scd.gsd.start asc")
    List<SlotCourseDetails> findByStandardIdOrderByGsdStart(Long classId);

    @Query("Select scd from SlotCourseDetails scd where scd.courseTeacher.teacher.id=?1 and scd.gsd.standard.term.id=?2 and scd.gsd.exam is null order by scd.gsd.start asc")
