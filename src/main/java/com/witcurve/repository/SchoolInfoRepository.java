...
public interface SchoolInfoRepository extends JpaRepository<SchoolInfo, Long> {
    ...
    @Query("select si from SchoolInfo si left join fetch si.school s left join fetch s.institute i order by s.primaryBranch desc, s.name asc, si.primaryBoard desc, si.board, i.name ")
    List<SchoolInfo> findAll(Long instituteId);

    @Query("select si from SchoolInfo si left join fetch si.school s left join fetch s.institute i where si.school.institute.id = (select school.institute.id from SchoolInfo where id = ?1) order by s.primaryBranch desc, s.name asc, si.primaryBoard desc, si.board ,i.name ")
