...
public class MessageThreadServiceImpl implements MessageThreadService {
    ...
    public Map<String, Integer> unReadCount(Long userId) throws WitcurveException {
        ...
        if (staff != null) {
            count = surveyFormRepository.findUnSubmittedFormCountForStaffByUserId(staff.getSchoolInfo().getId(), userId);
            result.put("SURVEY", count);
        }
        if (student != null) {
            count = surveyFormRepository.findUnSubmittedFormCountByUserId(student.getSchoolInfo().getId(), userId);
            result.put("SURVEY", count);
        }
        return result;
    }
